"""respx-driven tests for the synchronous SpotifyClient."""
...
@respx.mock
def test_rich_path_merges_tier1_and_embed() -> None:
...
@respx.mock
def test_degraded_path_returns_embed_track_with_warning(
...
@respx.mock
def test_401_invalidates_rebootstraps_and_retries() -> None:
...
@respx.mock
def test_dead_entity_raises_not_found_without_pathfinder() -> None:
    ...
    with SpotifyClient() as client, pytest.raises(NotFoundError):
        client.get_track(TRACK_ID)
...
def test_custom_transport_is_used() -> None:
...
def test_injected_transport_is_not_closed_by_client() -> None:
    transport = _RecordingTransport()
    with SpotifyClient(transport=transport):
        pass
    assert transport.closed is False


@respx.mock
def test_owned_transport_is_closed() -> None:
    respx.get(EMBED_URL).mock(return_value=httpx.Response(200, text=embed_html()))
    respx.get(PATHFINDER_RE).mock(return_value=httpx.Response(200, json=PATHFINDER_BODY))
