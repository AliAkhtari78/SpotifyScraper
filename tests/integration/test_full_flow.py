...
class TestFullFlow:
    ...
    def client(self):
        ...
        client = SpotifyClient(cookie_file=cookie_file, browser_type="requests", log_level="INFO")
    ...
    @pytest.mark.skipif(
        not os.environ.get("SPOTIFY_TEST_TRACK_URL"), reason="No test track URL provided"
    )
    def test_track_extraction(self, client):
        """Test extracting track information from a real Spotify URL."""
        track_url = os.environ.get(
            "SPOTIFY_TEST_TRACK_URL", "https://open.spotify.com/track/3n3Ppam7vgaVa1iaRUc9Lp"
        )
    ...
    @pytest.mark.skipif(
        not os.environ.get("SPOTIFY_TEST_ALBUM_URL"), reason="No test album URL provided"
    )
    def test_album_extraction(self, client):
        """Test extracting album information from a real Spotify URL."""
        album_url = os.environ.get(
            "SPOTIFY_TEST_ALBUM_URL", "https://open.spotify.com/album/4LH4d3cOWNNsVw41Gqt2kv"
        )
    ...
    @pytest.mark.skipif(
        not os.environ.get("SPOTIFY_TEST_ARTIST_URL"), reason="No test artist URL provided"
    )
    def test_artist_extraction(self, client):
        """Test extracting artist information from a real Spotify URL."""
        artist_url = os.environ.get(
            "SPOTIFY_TEST_ARTIST_URL", "https://open.spotify.com/artist/0YC192cP3KPCRWx8zr8MfZ"
        )
    ...
    @pytest.mark.skipif(
        not os.environ.get("SPOTIFY_TEST_PLAYLIST_URL"), reason="No test playlist URL provided"
    )
    def test_playlist_extraction(self, client):
        ...
        playlist_url = os.environ.get(
            "SPOTIFY_TEST_PLAYLIST_URL", "https://open.spotify.com/playlist/37i9dQZF1DXcBWIGoYBM5M"
        )
