...
- [ ] 1.1 In `src/spotify_scraper/api/pathfinder.py` add `SEARCH_OPERATION =
...
- [ ] 1.2 Add `build_search_url(query, *, variable_overrides=None)` mirroring
...
- [ ] 2.1 Create `src/spotify_scraper/models/search.py`: frozen slotted
...
- [ ] 2.2 Export `SearchResults` from `models/__init__.py` and the top-level
      `__init__.py` (import block + `__all__`, sorted).

## 3. Parser (sans-io)

- [ ] 3.1 Add `parse_search_results(union, *, query)` to `api/parse_entities.py`
...
- [ ] 3.2 Empty sections → empty tuples (NEVER raise `NotFoundError`); a section
...
- [ ] 4.1 Add to `_sync/client.py`: import `Sequence` from `collections.abc` and
...
- [ ] 4.2 Add `_search_union(query, types, limit)` and `_search_request(query,
...
- [ ] 4.3 Mirror 1:1 in `_async/client.py`: `async def search`, `async def
...
- [ ] 5.1 `tests/unit/api/test_pathfinder.py` (extend): `build_search_url` targets
...
- [ ] 5.2 `tests/unit/api/test_parse_search.py` (new): load `search.json`, call
...
- [ ] 5.3 `tests/unit/test_client_entities.py` + `..._async.py` (extend): respx
      ...
      `SpotifyScraperError`.
- [ ] 5.4 `tests/live/test_search.py` (new): `@pytest.mark.live`, credential-free,
...
- [ ] 6.1 `make lint`, `make type`, `make test`, `make cov` (>=85%) green;
      `test_parity.py` passes with `search` on both clients.
- [ ] 6.2 `make live` green (no cookie); confirm `search.json` still matches the
      live `searchV2` shape.
