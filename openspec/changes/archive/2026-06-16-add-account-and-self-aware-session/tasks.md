...
- [x] 0.2 Record the exact keys and value types actually returned: `product`,
...
- [x] 0.3 Confirm the non-200 split (expect 401 = bad/expired token → drives the
      retry). Note in design.md.
- [x] 0.4 Scrub the captured body (no `accessToken`/`clientId`/cookie/PII) into a
      synthetic fixture and pin it in design.md as the contract `parse_account`
      targets.

## 1. Account model

- [x] 1.1 Create `src/spotify_scraper/models/account.py`: frozen, slotted
...
- [x] 1.2 Export `Account` from `models/__init__.py` (import + `__all__`, first
...
- [x] 2.1 Create `src/spotify_scraper/api/account.py` mirroring `api/lyrics.py`:
      module-private `_PRODUCT_STATE_URL`, `product_state_url()` (NO arg),
      `auth_headers(token)`. Pure / I/O-free. Endpoint lives ONLY here.
- [x] 2.2 Add `parse_account(payload)` to `api/parse_entities.py`: import
...
- [x] 2.3 Add `_account_bool(value)` near `_optional_bool`: `None` if absent/
...
- [x] 3.1 Sync `_sync/client.py`: add imports `from spotify_scraper.api import
...
- [x] 3.2 Async `_async/client.py`: mirror 3.1 with `async def`, `await
...
- [x] 4.1 `auth/session.py`: add frozen, slotted `SessionInfo(exists, valid,
      ...
      `_default_now_ms`.
- [x] 4.2 `auth/session.py` `SessionStore`: add `info(*, path=None, now_ms=None)`
      and `has_session(*, path=None, now_ms=None)` delegating to file vs keyring
      backend, mirroring `save`/`load`/`clear`.
- [x] 4.3 `auth/session_keyring.py`: add `keyring_info(*, path=None, now_ms=None)`
...
- [x] 5.1 Sync `_sync/client.py` `login`: add `reuse: bool = True` keyword
      ...
      auto-skip sentence).
- [x] 5.2 Async `_async/client.py` `login`: mirror 5.1 with
      `capture_sp_dc_async` + `await`. `has_session`/`load` stay sync (no await).
- [x] 5.3 (Optional) Add a `session_info(*, store="file", session_path=None)`
...
- [x] 5.4 (Optional, only if surfaced publicly) Export `SessionInfo` from
...
- [x] 6.1 (Optional) In `browser/login.py`, surface the captured cookie's
...
- [x] 7.1 (Optional) Add a `session`/`status` command to `cli/main.py` printing
...
- [x] 8.1 NEW `tests/unit/api/test_parse_account.py`: table-test `parse_account`
...
- [x] 8.2 NEW `tests/unit/test_client_account.py` (respx, sync + async): reuse
...
- [x] 8.3 Extend a model round-trip test:
      `Account.from_dict(account.to_dict()) == account`, asserting `is_premium`
      is absent from `to_dict()`.
- [x] 8.4 Extend `tests/unit/auth/test_session.py`: `session_info`/
      ...
      cookie substring.
- [x] 8.5 Extend `tests/unit/test_client_login.py` (+ async mirror): `login(
...
- [x] 9.2 `make live` (with `SPOTIFY_SP_DC`) green; confirm captured product_state
      still matches `parse_account`/`_account_bool`.
- [x] 9.3 Docs: add `get_account`/`is_premium` to the authenticated-features page
...
- [x] 10.5 Docs: `get_account`/`is_premium`, `login(reuse=...)`, `session_info`/
