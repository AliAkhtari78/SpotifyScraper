...
class AsyncSpotifyClient:
    ...
    async def get_related_artists(self, value: str) -> tuple[Artist, ...]:
        ...
        """
        _, entity_id = self._resolve(value, "artist")
        union = await self._anon_union("artist_related", entity_id, "artistUnion")
    ...
    async def get_similar_albums(self, value: str, *, limit: int = 10) -> tuple[AlbumRef, ...]:
    ...
    async def get_user(self, user_id: str) -> UserProfile:
        ...
        token = await provider.token()
        try:
            return await self._fetch_user_profile(identifier, token)
        except AuthenticationError:
            provider.invalidate()
            return await self._fetch_user_profile(identifier, await provider.token())

    async def get_artist_events(self, value: str) -> tuple[Concert, ...]:
        ...
        """
        _, entity_id = self._resolve(value, "artist")
        node = await self._anon_union("artist_concerts", entity_id, "concerts")
        return parse_entities.parse_concerts(node)

    async def get_credits(self, value: str) -> Credits:
        ...
        token = await provider.token()
        try:
            return await self._fetch_credits(entity_id, token)
        except AuthenticationError:
            provider.invalidate()
            return await self._fetch_credits(entity_id, await provider.token())
    ...
    async def _anon_union(
        ...
        try:
            data = await self._anon_request(kind, entity_id, await self._tokens.token(), overrides)
        except TokenError:
            self._tokens.invalidate()
            data = await self._anon_request(kind, entity_id, await self._tokens.token(), overrides)
        union = data.get(union_key)
    ...
    async def _fetch_user_profile(self, user_id: str, token: str) -> UserProfile:
        url = user_profile_api.profile_url(user_id)
        try:
            response = await self._transport.get(url, headers=user_profile_api.auth_headers(token))
        except NotFoundError as exc:
    ...
    async def _fetch_credits(self, entity_id: str, token: str) -> Credits:
        url = credits_api.credits_url(entity_id)
        try:
            response = await self._transport.get(url, headers=credits_api.auth_headers(token))
        except NotFoundError as exc:
